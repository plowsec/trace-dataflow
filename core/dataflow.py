...
class DataFlowAnalyzer:
    ...
    def handle_store(self, stmt, tmp_values, tmp_taint, operand_map):
        ...
        if isinstance(stmt.data, pyvex.expr.RdTmp):
            ...
            if addr is not None:
                self.stack_variables[addr] = StackVariableOperand(OperandKind.SOURCE, addr, tmp_values.get(src_tmp),
                                                                  "unknown")
                if tmp_taint.get(src_tmp):
            ...
            if isinstance(stmt, pyvex.stmt.Store):
                ...
                if isinstance(stmt.data, pyvex.expr.RdTmp):
                    ...
                    if addr is not None and addr in self.stack_variables:
                        ...
                        self.digraph.add_node(operand_map[src_tmp].name)
                        self.digraph.add_node(addr)
                        self.digraph.add_edge(operand_map[src_tmp].name, addr,
                                              value=self.stack_variables[addr].value)
        ...
        for stmt in irsb.statements:
            if isinstance(stmt, pyvex.stmt.Put):
                ...
                if reg_name:
                    ...
                    # Add nodes and edges to the graph
                    self.digraph.add_node(mem_addr)
                    self.digraph.add_node(reg_name)
                    self.digraph.add_edge(mem_addr, reg_name, value=mem_value)
            ...
            if isinstance(stmt, pyvex.stmt.Put):
                ...
                if reg_name and reg_name in self.global_state['registers']:
                    ...
                    if mem_value in self.taint_map or mem_addr in self.taint_map:
                        ...
                        self.digraph.add_node(reg_name)
                        self.digraph.add_node(mem_addr)
                        self.digraph.add_edge(reg_name, mem_addr, value=mem_value)
                    else:
    ...
    def visualize_graph(self, layout='spring', figsize=(20, 20), node_size=500, font_size=8):
        ...
        if layout == 'spring':
            ...
            pos = nx.kamada_kawai_layout(self.digraph)
        else:
            pos = nx.spring_layout(self.digraph)  # Default to spring layout

        # Convert node labels to hexadecimal if they are numbers
        hex_labels = {node: hex(node) if isinstance(node, int) else node for node in self.digraph.nodes()}

        # Convert edge labels to hexadecimal if they are numbers
        edge_labels = nx.get_edge_attributes(self.digraph, 'value')
        hex_edge_labels = {(u, v): hex(data) if isinstance(data, int) else data for (u, v), data in edge_labels.items()}

        nx.draw(self.digraph, pos, labels=hex_labels, with_labels=True, node_size=node_size, node_color='lightblue',
                font_size=font_size, font_weight='bold')
        nx.draw_networkx_edge_labels(self.digraph, pos, edge_labels=hex_edge_labels, font_size=font_size)
        plt.show()

    # Function to export the taint flow graph to a file
